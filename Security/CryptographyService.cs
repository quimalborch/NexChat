...
namespace NexChat.Security
{
    ...
    public class CryptographyService : IDisposable
    {
        ...
        private void LoadOrCreateKeys()
        {
            try
            {
                ...
                else
                {
                    ...
                    Log.Warning("⚠️ [CRYPTO] Keys not found or incomplete, generating new key pair");
                    GenerateNewKeys();
                }
            }
            catch (Exception ex)
        ...
        private void GenerateNewKeys()
        {
            ...
            {
                Log.Warning(ex, "⚠️ [CRYPTO] Could not set restrictive permissions on private key file");
            }
        ...
        public EncryptedMessage EncryptMessage(string plaintext, RSA recipientPublicKey)
        {
            try
            {
                ...
                {
                    Log.Warning("⚠️ [CRYPTO] Plaintext is empty");
                }
        ...
        public string SignMessage(string message)
        {
            try
            {
                Log.Debug("✍️ [CRYPTO] Signing message...");
                Log.Debug("✍️ [CRYPTO] Message length: {Length} chars", message?.Length ?? 0);
                ...
                Log.Debug("✍️ [CRYPTO] Signature (Base64): {Size} chars", signatureBase64.Length);
        ...
        public bool VerifySignature(string message, string signatureBase64, RSA senderPublicKey)
        {
            try
            {
                ...
                {
                    Log.Warning("⚠️ [CRYPTO] Signature verification: INVALID (message may be tampered!)");
                }
